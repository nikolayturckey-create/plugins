...
public final class KitaListener implements Listener {
    ...
    public void onDamage(EntityDamageEvent event) {
        ...
        if (entity instanceof Cat cat) {
            double maxHealth = cat.getAttribute(org.bukkit.attribute.Attribute.GENERIC_MAX_HEALTH) != null
                    ? cat.getAttribute(org.bukkit.attribute.Attribute.GENERIC_MAX_HEALTH).getValue()
                    : 20.0;
            cat.setHealth(Math.min(maxHealth, Math.max(1.0, cat.getHealth())));
        }
    ...
    public void onInteract(PlayerInteractEntityEvent event) {
        ...
        event.setCancelled(true);
        kitaManager.playInteraction(event.getPlayer(), kitaManager.isOwner(event.getPlayer()));
    }
}
