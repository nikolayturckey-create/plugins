...
public final class KitaManager {
    ...
    public boolean isOwner(Player player) {
        ...
        return isDirectOwner(player) || player.hasPermission("kita.owner");
    }

    public boolean isDirectOwner(Player player) {
        return player != null && player.getName().equalsIgnoreCase(getOwnerName());
    }

    public boolean isFriend(Player player) {
        if (player == null || !plugin.getConfig().getBoolean("friends.enabled", true)) {
            return false;
        }
        for (String name : plugin.getConfig().getStringList("friends.names")) {
    ...
    public String getStateDescription() {
        ...
        if (following && getOwnerPlayer() != null) {
            return "следует за хозяином";
        }
        return "ждёт хозяина";
    }
    ...
    public void playInteraction(Player player, boolean owner) {
        ...
        if (owner) {
            ...
            cat.getWorld().spawnParticle(Particle.HEART, cat.getLocation().clone().add(0, 0.8, 0), 5, 0.3, 0.3, 0.3, 0.01);
        } else if (isFriend(player)) {
            player.sendMessage(CHAT_PREFIX + "узнаёт знакомого игрока и спокойно мурчит.");
            cat.getWorld().playSound(cat.getLocation(), Sound.ENTITY_CAT_PURR, 0.7F, 1.1F);
            cat.getWorld().spawnParticle(ThreadLocalRandom.current().nextBoolean() ? Particle.HEART : Particle.VILLAGER_HAPPY,
                    cat.getLocation().clone().add(0, 0.8, 0), 4, 0.3, 0.3, 0.3, 0.01);
        } else {
